...
Each game encompasses a **Hunt** for a dangerous and terrifying **Foe**, the **[Adversary](#foes)**, so the first step of setting up for a game is for the players to select an Adversary as the target of the Hunt. 
...
```{block, type = 'rnote'}
The entries for the Adversaries provide broad descriptions of these terrors but they are meant to be a source of inspiration, *not* strict representations. Use any Model you see fit to represent the Adversary and any other Model in the game. Interpreting the entries in your own way is a part of the fun!
```
...
Set up a Play Area that is  approximately 24” x 24” and place appropriate **[Terrain](#terrain)** for the encounter with the Adversary. If you are using the *Scouting & Preparation* phase as described in the *Narrative Rules* chapter, it might provide some guidance as to what Terrain is appropriate. 
...
Once all the Terrain has been placed, the players should take the time to discuss which pieces of Terrain are considered to be of each **Terrain** **Type**, and which sections of the **[Borders](#borders)** are considered **[Exits](#borders)**, to reduce confusion during play. 

### 4. Set Up Objectives {#objsetup .unnumbered}

*Cazadores* is most rewarding as a game when the players are also attempting to achieve additional **Objectives** beyond defeating the Adversary. For your first game, you might play without Objective but henceforth we recommend you use the *Objectives* rules in *Narrative Rules*. 
...
Mark the **Round** **Clock** to **Tick 1** and do the same for the Models’ **[Personal Clocks](#ticks)**. It’s possible that Scouting or Preparation might impact it so that some Models start at later Ticks, adjust accordingly. 
...
If the Round Clock would advance beyond Tick 10, instead it is reset to Tick 1 and the Round ends. When the Round ends, check for and resolve any effects that take place at the end of the Round. **Conditions** and Objectives are the most common sources of effects that are resolved at the end of the Round. 
...
- **The Pack Flees:** If all remaining Pack Models have fled the Play Area.
- **Objectives:** A special condition dictated by an Objective that ends the Hunt is fulfilled.

```{block, type = 'rnote'}
During campaigns, the outcome of the Hunt grants **Renown** based on the degree of success (or failure). To determine the Renown gained (or lost), see *Narrative Rules*.
```

### Executive Turns {#turns .unnumbered}
