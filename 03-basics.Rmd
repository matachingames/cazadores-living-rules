...
knitr::include_graphics(path = "images/scatter-roll.png", auto_pdf = TRUE)
```


Additionally, you might also be asked to roll a d100 to determine a random result on some tables, such as for [**Scars**](#scars). To do so, simply roll 2d10s and treat one of the dice as the *tens* and the other as the *ones* to obtain a result between 1 and 100.

## Actions, Ticks, & Clocks

### The Cost of Action {#ticks .unnumbered}

**[Cazadores](#cazadores)** are driven by boundless bravery and unshakeable resolve, while the **[Adversary](#adeck)** and its **[Minions](#ftypes)** possess unnatural strength and fearsome power. Neither are limited by their stamina or endurance during the **[Hunt](#structure)**. Instead, all **Actions** take precious time, represented as **Ticks**. 

Moments of furious struggle are demarcated by Ticks on the **[Round Clock](#rounds)**. A tracker with 10 Ticks that advances throughout the game. Each **Model** possesses a **Personal Clock** which tracks the next Tick on which they can act.

During a Model’s **[Turn](#turns)**, when it attempts an Action, the Model must first pay the Action’s Ticks, advancing their Personal Clock by that amount. Once it has carried out its Actions, the Model will not be able to take another Turn until the Round Clock matches their Personal Clock again.
...
- **Dangerous Terrain**: Craggy rocks, vine-covered jungles, all count as Dangerous Terrain. All Dangerous Terrain is treated as Difficult Terrain. In addition, the first time a Model enters Dangerous Terrain it must pass an **[Athletics Check](#checks)** or take the **Knocked Down [Condition](#conditions)**. 
...
- **Obscuring Terrain:** Fog, thick bush, etc., that provides **[Cover](#cover)** but does not block **[LOS](#los).**
...
The players’ **Models** are either **Cazadores**, the fearless and exceptional hunters who lead the **[Pack](#pack)** against insurmountable odds, or **Companions**, specialists, helpers, familiars, and reticent civilians who aid them on their travails. 
...
**Models** also have **Defence (DEF) Ratings** that indicate how well they are able to avoid or protect themselves from damage. In practice, these Ratings serve as the **[DT](#dt)** other Models must overcome when Attacking them. For the players’ Models, these Ratings are calculated as follows:
...
The **Foes** opposing the **Cazadores** are defined with less detail but with all the **Attributes** required to be worthy opponents. They are either **Minions** or **Adversaies**. **Foes**’ **Actions** are determined using **Foe Type**, **Decision Trees**, and the **Adversary Deck**.
...
- **Terror (TER):** Unnatural power, used to terrify and clear **[Conditions](#conditions)**.
...
- **Sentry (♣):** Models that do not focus on engaging the Cazadores in combat and instead patrol **[Objectives](obj)** or positions near the Adversary.

### Foe Move and Recover Actions {#fmove .unnumbered}

Like other Models, Foes also receive a **[free Move Action](#actions)**, therefore, the first Move Action is often free. However, Foes can take the **Recover Action** instead of the free Move Action, and will always do so to clear Conditions before resolving **Decision Trees**. Foes will also always move as far as possible as whenever they execute a Move Action.
...
### Adversaries and The Adversary Deck {#adeck .unnumbered}
...
Each **Adversary** has a detailed entry for their Adversary Deck that describes specific Hit Locations and Actions tailored to fit their nature and particular qualities, these can be found in *[The Adversary](#adversaryintro)* book but you can find one in the *[Quickstart](#quickstart)* chapter.

#### Hit Locations {-}

All Adversaries have their total number of **[Wounds (WNDs)](#wounds)** spread out across various Hit Locations. Whenever the players’ Models successfully attack the Adversary, they flip a number of Hit Location cards equal to WNDs dealt, and the WNDs are removed from that specific Hit Location.
...
The suit determines which **[Foe Type](#ftype)** the Adversary will use this Turn:
